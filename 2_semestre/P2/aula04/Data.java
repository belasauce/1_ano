...
public class Data {
  ...
  public static Data parseData(String data){
    ...
    if(num.length == 3){
      int dia1 = Integer.parseInt(num[2]);
      int mes1 = Integer.parseInt(num[1]);
      int ano1 = Integer.parseInt(num[0]);

      return new Data(dia1, mes1, ano1);
    }
    else{
      return null;
    }
  ...
  public int compareTo(Data comp){
    if (comp.dia() == dia && comp.mes() == mes && comp.ano() == ano){
      return 0;
    }
    else if((comp.ano() > ano) || (comp.ano() == ano && comp.mes() > mes) || (comp.ano() == ano && comp.mes() == mes && comp.dia() > dia)){
      return -1;
    }
    else{
      return 1;
